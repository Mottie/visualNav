 ...
 * MIT licensed.
 */
;(function(c){c.visualNav=function(h,k){var b,a=this;a.$el=c(h);a.$el.data("visualNav",a);a.init=function(){var d;a.initialized=!1;a.options=b=c.extend({},c.visualNav.defaultOptions,k);a.winLoc=window.location;a.history="history"in window?window.history:null;a.$win=c(window);a.$doc=c(document);a.$body=c("html, body");a.curHash="";a.$lastItem=[null];a.namespace=".visualNav";c.each(b.easing,function(a,d){c.isFunction(c.easing[d])||(b.easing[a]="swing")});b.stopOnInteraction&&a.$body.bind("scroll mousedown DOMMouseScroll mousewheel keyup ".split(" ").join(a.namespace+
